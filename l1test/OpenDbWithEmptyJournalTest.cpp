...
class ADbWithEmptyJournal : public Test {
protected:
    sqlite3* db;
    void SetUp() override
    {
...
TEST_F(ADbWithEmptyJournal, ReturnsCantopen14orError1orOkIfOpen)
{
...
TEST_F(ADbWithEmptyJournal, IsEmptyIfOpen)
{
    ASSERT_THAT(sqlite3_open(kPath, &db), Eq(SQLITE_OK));
    EXPECT_THAT(sqlite3_exec(db, kIntegrityCheck, 0, 0, 0), Eq(SQLITE_OK));
    EXPECT_THAT(sqlite3_exec(db, kSchema, 0, 0, 0), Eq(SQLITE_CANTOPEN));
...
TEST_F(ADbWithEmptyJournal, ReturnsCantopen14orError1orOkIfOpenEmpty)
{
